    ...
    foreach ($menu as $section) {
      ...
      echo '<div class="accordion-item">';
        ...
        echo '<h3 class="accordion-header" id="'.htmlspecialchars($hid).'">';
          $btnClasses = 'accordion-button'.$btnCollapsed.' bip-borderable'.($sectionActive ? ' active' : '');
          ...
          echo '<div class="accordion-body p-0">';
            echo '<ul class="list-group'.($listFlush?' list-group-flush':'').'">';
              ...
              foreach ($items as $item) {
                ...
                $liCls  = 'list-group-item'.($active ? ' active' : '');
                $aCls   = 'bip-link bip-borderable'.($active ? ' active' : '');
                $aria   = $active ? ' aria-current="page"' : '';
                $dismiss = $dismissOnClick ? ' data-bs-dismiss="offcanvas"' : '';
                $url = bip_href($href);

                echo '<li class="'.$liCls.'">';
                  echo '<a class="'.$aCls.'" href="'.htmlspecialchars($url).'"'.$dismiss.$aria.'>'
